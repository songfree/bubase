// arb.cpp : Defines the entry point for the console application.
//
...
#pragma message("Automatically linking with   bf_kerneld.lib bf_xdpd.lib bf_drebapid.lib")
#else
#pragma comment(lib, "bf_kernel.lib") 
#pragma comment(lib, "bf_xdp.lib") 
#pragma comment(lib, "bf_drebapi.lib") 
#pragma message("Automatically linking with   bf_kernel.lib bf_xdp.lib bf_drebapi.lib")
#endif
...
int main(int argc, char* argv[])
{
	...
	{
		printf("命令行参数说明:\n");
		printf("               不带参数 后台运行 配置文件为testarb.xml \n");
		printf("               stop/STOP  退出testarb \n");
		printf("               -cf  前台运行 \n");
		...
		printf("               -mq  退出正在运行的程序 \n");
		printf("               -f   设置配置文件 -ftmc.xml配置文件为tmc.xml  默认配置文件为testarb.xml \n");
		printf("               -q   退出 \n");
	}
...
#ifdef _WINDOWS
	CBF_Md5 md5class;
	sprintf(shmname,"%s\\testarb",frun.m_pRes.g_sCurPath);
	md5class.Update(shmname);
	sprintf(shmname,"testarb_%s",md5class.ToString().c_str());
#else
	sprintf(shmname,"%s/testarb",frun.m_pRes.g_sCurPath);
#endif
	...
	g_pExitFlag[0] = 1;
	g_pExitFlag[2] = ((CBF_LogClient *)frun.m_pLog)->GetLogLevel();
	g_pExitFlag[3] = frun.m_pDrebApi.GetDataLogLevel();
	while (true)
	{
		if (g_pRunFlag == 0)
		{
			printf("------begin exit testarb by signal.......\n");
			frun.m_pRes.g_bToExit = true;
			frun.Stop();
			printf("------exit testarb by signal ok !\n");
			SLEEP(100);
			g_pExitFlag[0] = 0;
			g_pExitFlag = NULL;
			return 0;
		...
		if (g_pExitFlag[0] != 1)
		{
			printf("------begin exit testarb.......\n");
			frun.m_pRes.g_bToExit = true;
			//frun.m_pRes.WriteBack();
			frun.Stop();
			printf("------exit testarb ok !\n");
			SLEEP(100);
			g_pExitFlag = NULL;
			return 0;
		}
		if (g_pExitFlag[1] >0) //日志参数有变动
		{
			if (g_pExitFlag[1] == 2)
			{
				((CBF_LogClient *)frun.m_pLog)->SetLogLevel(g_pExitFlag[2]);
			}
