...
{	
    ...
    // 编程  : 王明松 2014-10-30 11:31:20
    // 返回  : unsigned short 
    // 参数  : unsigned int xdpindex   xdp类的下标，由InitXdp获得
    // 描述  : 取得当前实际报文的大小
    CXDP_EXPORT unsigned short XDPI_GetRealPackSize( int xdpindex);
    ...
    // 参数  : const char *fieldname
    // 描述  : 取消字段，这样就不会生成此字段报文或取出值了
    CXDP_EXPORT int XDPI_CancelFieldByName(unsigned int xdpindex, unsigned short rec, const char* fieldname);
    ...
    // 参数  : unsigned short index
    // 描述  : 取消字段，这样就不会生成此字段报文或取出值了
    CXDP_EXPORT int XDPI_CancelFieldByIndex(unsigned int xdpindex, unsigned short rec, unsigned short index);
    ...
    // 参数  : char *xdpbuf
    // 描述  : 将报文内容显示出来，二进制数据转为16进制显示
    CXDP_EXPORT void XDPI_PrintXdp(unsigned int xdpindex, char* xdpbuf);
    ...
    // 参数  : unsigned int xdpindex   xdp类的下标，由InitXdp获得
    // 描述  : 取得保存多记录的结构数目，注意不包括第一条记录
    CXDP_EXPORT unsigned short XDPI_GetCurSavePackNum(unsigned int xdpindex);
    ...
    // 参数  : unsigned int xdpindex   xdp类的下标，由InitXdp获得
    // 描述  : 清空保存多记录的结构
    CXDP_EXPORT void XDPI_ClearNextRecord(unsigned int xdpindex);
    ...
    // 参数  : const char *fieldname/unsigned short index 字段名称/索引
    // 描述  : 取得字段类型
    CXDP_EXPORT unsigned short XDPI_GetFieldTypeByName(unsigned int xdpindex, const char* fieldname);
    CXDP_EXPORT unsigned short XDPI_GetFieldTypeByIndex(unsigned int xdpindex, unsigned short index);
    ...
    // 参数  : unsigned int xdpindex   xdp类的下标，由InitXdp获得
    // 描述  : 返回当前报文中共有几条记录，注意记录数和保存多记录的结构数目不是一个概念
    CXDP_EXPORT unsigned short XDPI_GetRecCount(unsigned int xdpindex);
    ...
    // 参数  : char *errmsg  出错信息
    // 描述  : 从xdp中取字段值
    CXDP_EXPORT int XDPI_GetFieldValueByName(unsigned int xdpindex, unsigned short rec, const char* fieldname, char* data, unsigned int* datalen, int* fieldtype, char* errmsg);
    CXDP_EXPORT int XDPI_GetFieldValueByIndex(unsigned int xdpindex, unsigned short rec, unsigned short index, char* data, unsigned int* datalen, int* fieldtype, char* errmsg);
    ...
    // 参数  : char *errmsg   出错信息
    // 描述  : 从xdp中取第一个记录的字段值
    CXDP_EXPORT int XDPI_GetFirstFieldValueByIndex(unsigned int xdpindex, unsigned short index, char* data, unsigned int* datalen, int* fieldtype, char* errmsg);
    CXDP_EXPORT int XDPI_GetFirstFieldValueByName(unsigned int xdpindex, const char* fieldname, char* data, unsigned int* datalen, int* fieldtype, char* errmsg);
    ...
    // 参数  : char *errmsg    出错信息
    // 描述  : 设置指定记录数的字段值
    CXDP_EXPORT int XDPI_SetFieldValueByName(unsigned int xdpindex, unsigned short rec, const char* fieldname, const char* data, unsigned int datalen, char* errmsg);
    CXDP_EXPORT int XDPI_SetFieldValueByIndex(unsigned int xdpindex, unsigned short rec, unsigned short index, const char* data, unsigned int datalen, char* errmsg);
    ...
    // 参数  : char *errmsg  出错信息
    // 描述  : 设置第一条记录的字段值
    CXDP_EXPORT int XDPI_SetFirstFieldValueByIndex(unsigned int xdpindex, unsigned short index, const char* data, unsigned int datalen, char* errmsg);
    CXDP_EXPORT int XDPI_SetFirstFieldValueByName(unsigned int xdpindex, const char* fieldname, const char* data, unsigned int datalen, char* errmsg);
    ...
    // 参数  : unsigned short rec 
    // 描述  : 设置当前的记录数，当原报文中有Y条记录时，本次只返回X(X<Y)条记录时，必须重置记录数为X,否则返回的仍为Y条
    CXDP_EXPORT int XDPI_SetCurRecCount(unsigned int xdpindex, unsigned short x);
    ...
    // 参数  : unsigned int xdpindex xdp类的下标，由InitXdp获得
    // 描述  : 清除多记录，重置位图
    CXDP_EXPORT void XDPI_ResetData(unsigned int xdpindex);
    ...
    // 参数  : char *errmsg   出错信息
    // 描述  : 生成xdp报文
    CXDP_EXPORT int XDPI_ToBuffer(unsigned int xdpindex, char* xdpbuf, unsigned int* len, char* errmsg);
    ...
    // 参数  : char *errmsg   出错信息
    // 描述  : 从数据缓冲解析报文
    CXDP_EXPORT int XDPI_FromBuffer(unsigned int xdpindex, const char* xdpbuf, unsigned int xdpbuflen, char* errmsg);
    ...
    // 参数  : char *errmsg      出错信息
    // 描述  : 初始化报文，
    CXDP_EXPORT int XDPI_InitXdp(const char* datafile, char* errmsg);
    ...
    // 参数  : unsigned int xdpindex xdp类的下标，由InitXdp获得
    // 描述  : 释放xdp类的实例
    CXDP_EXPORT void XDPI_FreeXdp(unsigned int xdpindex);


    // 函数名:  XDP_Copy
    // 编程  : 王明松 2014-11-11 10:22:21
    // 返回  : CXDP_EXPORT int 0成功 非0失败
    // 参数  : unsigned int destindex xdp类的下标，由InitXdp获得  目标
    // 参数  : unsigned int srcindex  xdp类的下标，由InitXdp获得   源
    // 描述  : xdp复制
    CXDP_EXPORT int  XDPI_Copy(unsigned int destindex, unsigned int srcindex);
