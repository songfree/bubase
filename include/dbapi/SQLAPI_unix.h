...
#ifndef  SQLAPI_WIN32
	#ifdef SS_64BIT_SERVER   //64位机器
  	typedef long long sa_int64_t;
 		typedef unsigned long long sa_uint64_t;
	#else                    //32位机器
		typedef int64_t sa_int64_t;
		typedef unsigned long long sa_uint64_t;
	#endif  // !SS_64BIT_SERVER
#endif  // !SQLAPI_WIN32
...
public:
	...
	SANumeric(sa_int64_t val);			// initializes from 64-bit integer
	SANumeric(sa_uint64_t val);
...
public:
	...
	void mysql_character(char *set_name);
};
