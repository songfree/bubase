...
{
	std::string sdrebip;//服务ip
	unsigned int nPort;//端口
	unsigned short nNodeId; //dreb公共节点
	unsigned int   cNodePrivate;//dreb私有节点
	unsigned int   nSvrId;//公共服务号
	unsigned int   cSvrPrivate;//私有服务号
	unsigned int   nServiceNo;//服务功能号
	unsigned int   nConnectType;//连接类型
	unsigned int   nThreadNum;//启动线程数
	unsigned int   cZip;//数据标志
	unsigned int   nStartTime;//启动时间点
	int            nCmd;//总线命令字
	unsigned int   nSendSep;//发送间隔
};
