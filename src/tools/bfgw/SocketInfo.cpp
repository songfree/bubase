...
void CSocketInfo::OnClose(const char *msg,const char *filename,int fileline)
{
	...
	//清空队列
	if (m_nType == GW_SOCK_IN || m_nType == GW_SOCK_OUT)
	{
	...
	{
		m_log.LogMp(LOG_DEBUG,filename,fileline,"%s,关闭连接[%s:%d] index[%d] type[%d] sock[%d]",\
		msg,m_sDrebIp.c_str(),m_sock,m_index,m_nType,m_sock);
	}
...
// 		if (m_nType == GW_SOCK_OUT)
// 		{
// 			FILE *fp = fopen("rcvdata.dat","ab");
// 			if (fp != NULL)
// 			{
// 				fwrite(m_sRcvBuffer+m_nRcvBufLen,1,ret,fp);
// 				fclose(fp);
// 			}
// 		}
// 		
...
{
	if (m_sock != INVALID_SOCKET && (m_nType == GW_SOCK_IN || m_nType == GW_SOCK_OUT))
	{
		return true;
	}
	return false;
...
void CSocketInfo::SetOutput(const char *ip,int port)
{
	m_nType = GW_SOCK_OUT;
	m_bNeedread = true;
	m_bNeedConnect = true;
