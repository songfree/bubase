...
int main(int argc, char* argv[])
{
    ...
    sprintf(g_VERSION, "2.0.6 %s", __DATE__);
	if (argc == 1)
	{
		...
		printf("               -mq  退出正在运行的程序 \n");
		printf("               -f   设置配置文件 -ftmc.xml配置文件为tmc.xml  默认配置文件为bfcgate.xml \n");
		printf("               -q   退出 \n");
	}
	
	CBF_Tools::GetModuleNamePath(argv,g_pGWRun.g_pRes.g_sCurPath,g_pGWRun.g_pRes.g_sModulePath,g_pGWRun.g_pRes.g_sModuleName);
	
#ifdef _WINDOWS
	CBF_Md5 md5class;
	sprintf(shmname,"%s\\cgate",g_pGWRun.g_pRes.g_sCurPath);
	md5class.Update(shmname);
	sprintf(shmname,"cgate_%s",md5class.ToString().c_str());
#else
	sprintf(shmname,"%s/cgate",g_pGWRun.g_pRes.g_sCurPath);
#endif
	...
	g_pExitFlag = (char *)pShare.Open(shmname,4);
	if (g_pExitFlag == NULL)
	{
		printf("打开共享内存错\n");
		return -1;
...
// 	}
	...
	//读取配置文件
	if (!g_pGWRun.Init(confile))
	{
		printf("初始化失败 %s\n",g_pGWRun.GetErrMsg().c_str());
		return -1;
	}
	if (!g_pGWRun.Start())
	{
	...
	while (true)
	{
		if (g_pRunFlag == 0)
		{
			printf("------begin exit bfcgate by signal.......\n");
			g_pExitFlag[0] = 0;
			g_pGWRun.Stop();
			printf("------exit bfcgate by signal ok !\n");
			SLEEP_SECONDS(5);
			return 0;
		}
		if (g_pExitFlag[0] != 1)
		{
			printf("------begin exit bfcgate.......\n");
			g_pGWRun.Stop();
			printf("------exit bfcgate ok !\n");
			SLEEP_SECONDS(5);
			return 0;
		}
