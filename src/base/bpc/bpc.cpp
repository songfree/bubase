...
int main(int argc, char* argv[])
{
	...
	if (argc == 1)
	{
		...
		printf("               -mq  退出正在运行的程序 \n");
		printf("               -f   设置配置文件 -ftmc.xml配置文件为tmc.xml  默认配置文件为bfsap.xml \n");
		printf("               -q   退出 \n");
	}
...
#ifdef _WINDOWS
	CBF_Md5 md5class;
	sprintf(shmname,"%s\\bfsap",frun.m_pRes.g_sCurPath);
	md5class.Update(shmname);
	sprintf(shmname,"bfsap_%s",md5class.ToString().c_str());
#else
	sprintf(shmname,"%s/bfsap",frun.m_pRes.g_sCurPath);
#endif
...
#ifdef _WINDOWS
	printf("共享内存id[%d] 运行标志[%d] 缓冲大小[%ld]\n",pShare.m_shmid,g_pExitFlag[0],BPCBUFSIZE);
#else
	printf("共享内存id[%d] 文件索引ID[%s] 运行标志[%d] 缓冲大小[%ld]\n",pShare.m_shmid,pShare.m_szmsg,g_pExitFlag[0],BPCBUFSIZE);
#endif
...
// 	}
	...
	{
#ifdef _WINDOWS
		sprintf(shmname,"PID[%d] 共享内存id[%d] 缓冲大小[%ld]",GetPid(),pShare.m_shmid,BPCBUFSIZE);
#else
		sprintf(shmname,"PID[%d] 共享内存id[%d] 文件索引ID[%s] 缓冲大小[%ld]",GetPid(),pShare.m_shmid,pShare.m_szmsg,BPCBUFSIZE);
#endif
	...
	while (true)
	{
		if (g_pRunFlag == 0)
		{
			printf("------begin exit bfsap by signal.......\n");
			frun.m_pRes.g_bToExit = true;
			frun.Stop();
			printf("------exit bfsap by signal ok !\n");
			SLEEP(100);
			g_pExitFlag[0] = 0;
			g_pExitFlag = NULL;
			return 0;
		...
		if (g_pExitFlag[0] != 1)
		{
			printf("------begin exit bfsap.......\n");
			frun.m_pRes.g_bToExit = true;
			//frun.m_pRes.WriteBack();
			frun.Stop();
			printf("------exit bfsap ok !\n");
			SLEEP(100);
			g_pExitFlag = NULL;
			return 0;
		}
