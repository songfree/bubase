...
int main(int argc, char* argv[])
{
	...
	{
		printf("命令行参数说明:\n");
		printf("               不带参数 后台运行 配置文件为bfarb.xml \n");
		printf("               stop/STOP  退出bfarb \n");
		printf("               -cf  前台运行 \n");
		...
		printf("               -mq  退出正在运行的程序 \n");
		printf("               -f   设置配置文件 -ftmc.xml配置文件为tmc.xml  默认配置文件为bfarb.xml \n");
		printf("               -q   退出 \n");
	}
...
#ifdef _WINDOWS
	CBF_Md5 md5class;
	sprintf(shmname,"%s\\bfarb",frun.m_pRes.g_sCurPath);
	md5class.Update(shmname);
	sprintf(shmname,"bfarb_%s",md5class.ToString().c_str());
#else
	sprintf(shmname,"%s/bfarb",frun.m_pRes.g_sCurPath);
#endif
...
// 	}
	...
	while (true)
	{
		if (g_pRunFlag == 0)
		{
			printf("------begin exit bfarb by signal.......\n");
			frun.m_pRes.g_bToExit = true;
			frun.Stop();
			printf("------exit bfarb by signal ok !\n");
			SLEEP(100);
			g_pExitFlag[0] = 0;
			g_pExitFlag = NULL;
			return 0;
		...
		if (g_pExitFlag[0] != 1)
		{
			printf("------begin exit bfarb.......\n");
			frun.m_pRes.g_bToExit = true;
			//frun.m_pRes.WriteBack();
			frun.Stop();
			printf("------exit bfarb ok !\n");
			SLEEP(100);
			g_pExitFlag = NULL;
			return 0;
		}
