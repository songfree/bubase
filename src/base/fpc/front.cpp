// front.cpp : Defines the entry point for the console application.
//
...
int main(int argc, char* argv[])
{
	...
	{
		printf("命令行参数说明:\n");
		printf("               不带参数 后台运行 配置文件为bffpc.xml \n");
		printf("               stop/STOP  退出bffpc \n");
		printf("               -cf  前台运行 \n");
		printf("               -cb  后台运行 无-c时默认为后台运行 \n");
		printf("               -m   设置共享内存参数 \n");
		printf("               -ml  设置日志级别   -ml5设置日志级别为5，即调试 \n");
		printf("               -mq  退出正在运行的程序 \n");
		printf("               -f   设置配置文件 -ftmc.xml配置文件为tmc.xml  默认配置文件为bffpc.xml \n");
		printf("               -q   退出 \n");
	}
...
#ifdef _WINDOWS
	CBF_Md5 md5class;
	sprintf(shmname,"%s\\bffpc",frun.g_sCurPath);
	md5class.Update(shmname);
	sprintf(shmname,"bffpc_%s",md5class.ToString().c_str());
#else
	sprintf(shmname,"%s/bffpc",frun.g_sCurPath);
#endif
	...
	g_pExitFlag = (char *)pShare.Open(shmname,5);
	if (g_pExitFlag == NULL)
	{
		printf("打开共享内存错\n");
		return -1;
...
#ifdef _WINDOWS
	printf("共享内存id[%d] 运行标志[%d] 缓冲大小[%ld]\n",pShare.m_shmid,g_pExitFlag[0],BFBUFSIZE);
#else
	printf("共享内存id[%d] 文件索引ID[%s] 运行标志[%d] 缓冲大小[%ld]\n",pShare.m_shmid,pShare.m_szmsg,g_pExitFlag[0],BFBUFSIZE);
#endif
...
// 	}
	...
	if ( (ret=fork()) == 0) 
	{
		...
		{
#ifdef _WINDOWS
			sprintf(shmname,"PID[%d] 共享内存id[%d] 缓冲大小[%ld]",GetPid(),pShare.m_shmid,BFBUFSIZE);
#else
			sprintf(shmname,"PID[%d] 共享内存id[%d] 文件索引ID[%s] 缓冲大小[%ld]",GetPid(),pShare.m_shmid,pShare.m_szmsg,BFBUFSIZE);
#endif
			fwrite(shmname,1,strlen(shmname),fp);
			fclose(fp);
		}

		if (!frun.InitFront(confile,g_pExitFlag))
		{
		...
		while (true)
		{
			if (g_pExitFlag[0] != 1)
			{
				printf("------begin exit bffpc.......\n");
				frun.m_log->LogMp(LOG_ERROR,__FILE__,__LINE__,"开始退出bffpc");
				frun.m_pRes.g_bToExit = true;
				//frun.m_pRes.WriteBack();
				frun.StopFront();
				frun.m_log->LogMp(LOG_ERROR,__FILE__,__LINE__,"成功退出bffpc");
				printf("------exit bffpc ok !\n");
				SLEEP(100);
				g_pExitFlag = NULL;
				return 0;
			}
