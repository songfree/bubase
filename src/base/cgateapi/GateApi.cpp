...
std::vector<CGateLink*> g_GateApiArray;
...
{
	if (clientid > g_GateApiArray.size() - 1)
	{
		return -1;
	}
    CGateLink* pGl = (CGateLink*)g_GateApiArray[clientid];
    return pGl->GetConnectStatus();
}
...
void BFCGATE_TSI_Logout(unsigned int clientid)
{
    ...
    pGl->Logout();
    delete pGl;
	g_GateApiArray[clientid] = NULL;
	return;
}

// 函数名: BFCGATE_TSI_InitApi
// 编程  : 王明松 2013-5-23 14:49:22
// 返回  : int >=0 成功 网关连接类下标 <0失败
// 参数  : const char *apixml xml配置文件
// 描述  : 初始化api，读取配置文件，得到证书目录及网关地址端口，并连接网关
int BFCGATE_TSI_InitApi(const char* apixml)
{
	CGateLink *pClass = new CGateLink();
    if (pClass == NULL)
    {
        return -1;
    }
